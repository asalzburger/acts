...
ActsExamples::Geant4::SimulationTrackingAction::SimulationTrackingAction(
    const Config& cfg, std::unique_ptr<const Acts::Logger> logger)
    : G4UserTrackingAction(), m_cfg(cfg), m_logger(std::move(logger)) {}

void ActsExamples::Geant4::SimulationTrackingAction::PreUserTrackingAction(
    const G4Track* aTrack) {
  auto g4RunManager = G4RunManager::GetRunManager();
  unsigned int eventID = g4RunManager->GetCurrentEvent()->GetEventID();

  EventStoreRegistry::particlesInitial[eventID].push_back(convert(*aTrack));
}

void ActsExamples::Geant4::SimulationTrackingAction::PostUserTrackingAction(
    const G4Track* aTrack) {
  auto g4RunManager = G4RunManager::GetRunManager();
  unsigned int eventID = g4RunManager->GetCurrentEvent()->GetEventID();

  EventStoreRegistry::particlesFinal[eventID].push_back(convert(*aTrack));
}

ActsExamples::SimParticle
ActsExamples::Geant4::SimulationTrackingAction::convert(
