...
ActsExamples::Geant4::SimulationSteppingAction::SimulationSteppingAction(
    const Config& cfg, std::unique_ptr<const Acts::Logger> logger)
    : G4UserSteppingAction(), m_cfg(cfg), m_logger(std::move(logger)) {}

void ActsExamples::Geant4::SimulationSteppingAction::UserSteppingAction(
    const G4Step* step) {
  ...
  if (volumeName.find(m_cfg.sensitivePrefix) != std::string::npos) {
    ...
    G4ThreeVector preStepMomentum = convertEnergy * preStepPoint->GetPosition();
    G4double preStepEnergy = convertEnergy * preStepPoint->GetTotalEnergy();
    G4ThreeVector postStepMomentum =
        convertEnergy * postStepPoint->GetPosition();
    ...
    volumeName.erase(0, m_cfg.sensitivePrefix.size());
    ...
    EventStoreRegistry::hits[eventNr].emplace_back(
        geoID, particleID, particlePosition, beforeMomentum, afterMomentum,
        EventStoreRegistry::hits[eventNr].size());
  }
}
