...
ActsExamples::ProcessCode ActsExamples::DigitizationAlgorithm::execute(
  ...
  for (auto [moduleGeoId, moduleSimHits] : groupByModule(simHits)) {
    ...
    for (auto h = moduleSimHits.begin(); h != moduleSimHits.end(); ++h) {
      // @todo in follow-up PR: allow for channel merging
      // std::vector<ActsFatras::Channelizer::ChannelSegment> moduleSegments;

      const auto& simHit = *h;
      const auto simHitIdx = simHits.index_of(h);

      // @todo: Get the drift direction from a lorentz angle emulation
      Acts::Vector3 driftDir(0., 0., 1.);

      auto depletion = segmentationItr->depletion;
      auto segmentation = segmentationItr->segmentation;

      // Bring the hit into the local coordinate frame by drifting
      auto readoutSegment = m_planarDrift.toReadout(
          ctx.geoContext, *surfacePtr, depletion, simHit.position(),
      ...
      if (maskedSegment.ok()) {
        // Channelize it
        auto channelSegments = m_channelizer.segments(
            ctx.geoContext, *surfacePtr, segmentation, maskedSegment.value());
      }
    }
  }
