  ...
  for (Index hitIdx = 0u; hitIdx < measurements.size(); ++hitIdx) {
    ...
    std::visit(
        [&](const auto& m) {
          ...
          meas.cov0 = covariance(Acts::eBoundLoc0, Acts::eBoundLoc0);
          meas.cov1 = covariance(Acts::eBoundLoc1, Acts::eBoundLoc1);
          meas.covPhi = covariance(Acts::eBoundPhi, Acts::eBoundPhi);
          meas.covTheta = covariance(Acts::eBoundTheta, Acts::eBoundTheta);
          meas.covTime = covariance(Acts::eBoundTime, Acts::eBoundTime);
          ...
          if (not clusters.empty()) {
            ...
            cell.association_id = meas.measurement_id;
            for (auto& c : cluster.channels) {
              cell.channel0 = c.bin[0];
              cell.channel0 = c.bin[1];
          ...
          truth.measurement_id = meas.measurement_id;
