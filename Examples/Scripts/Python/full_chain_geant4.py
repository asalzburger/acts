...
# Full chain with the OpenDataDetector, simulated with Geant4 and
# reconstructed with a CKF whose propagator uses the Geant4Stepper:
# trajectory and covariance transport during track finding are performed by
# the Geant4(e) machinery through the Geant4 geometry, sharing the Geant4
# setup with the simulation. The fitter's internal material handling
# (multiple scattering / energy loss) is switched off accordingly, as the
# Geant4 stepper applies the material effects itself.
#
# Geant4 is not thread safe - this chain runs strictly single-threaded.
...
parser = argparse.ArgumentParser(
    description="Full chain with the OpenDataDetector: Geant4 simulation and "
    "track finding with the Geant4 stepper (single-threaded)"
)
parser.add_argument(
    "--output",
    "-o",
...
# Geant4 (simulation and the Geant4 stepper) is not thread safe.
# FPE tracking is disabled: the lazy initialization of the Geant4e energy
# loss tables (G4TablesForExtrapolator) raises a benign FLTINV deep inside
# Geant4, which cannot be masked by source location.
...
if args.reco:
    ...
    # Track finding with the Geant4 stepper: the propagator of the CKF
    # performs parameter and covariance transport with Geant4(e) through the
    # Geant4 geometry of the simulation above. Material effects are applied
    # by Geant4 itself, so the internal material handling of the fitter is
    # switched off (multipleScattering/energyLoss in CkfConfig).
    #
    # Note: makeGeant4TrackFinderFunction has to be called after addGeant4
    # (it shares the Geant4 setup of the simulation).
    from acts.examples.geant4 import makeGeant4TrackFinderFunction

    g4FindTracks = makeGeant4TrackFinderFunction(
        trackingGeometry,
        field,
        acts.logging.INFO,
        meanEnergyLoss=True,
        covarianceNoise=True,
    )
    ...
    addCKFTracks(
        ...
        CkfConfig(
            ...
            maxPixelHoles=1,
            maxStripHoles=2,
            # material handling is done by the Geant4 stepper
        ...
        findTracksFunction=g4FindTracks,
        outputDirRoot=outputDir if args.output_root else None,
        outputDirCsv=outputDir if args.output_csv else None,
        writeCovMat=True,
    )
