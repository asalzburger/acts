...
BOOST_AUTO_TEST_CASE(MaterialTesting) {
  ...
  BOOST_REQUIRE_NE(negDiscMat, nullptr);
  ...
  BOOST_REQUIRE_NE(posDiscMat, nullptr);
  ...
  const auto* outerCylMat = dynamic_cast<const ProtoSurfaceMaterial*>(outerCyl);
  BOOST_REQUIRE_NE(outerCylMat, nullptr);
...
BOOST_AUTO_TEST_CASE(MaterialCuboid) {
  ...
  for (std::size_t i = 0; i < child.portals().size(); i++) {
    ...
    BOOST_REQUIRE_NE(gridMaterial, nullptr);
