...
class GridSurfaceMaterialAccumulater final
 ...
 public:
  ...
  struct State final : public ISurfaceMaterialAccumulater::State {
    ...
    /// Surface lookup for lazy accumulation initialization
    std::map<GeometryIdentifier, const Surface*> materialSurfaces;

    /// Indicator whether accumulation bins are initialized
    bool accumulationInitialized = false;
  };
  ...
  /// @param gctx is the geometry context for this accumulation step
